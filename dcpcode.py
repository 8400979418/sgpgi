...
if newpass==cpass and oldpass==p and st=="true":
	...
	n=cur.execute(q)
	if n==1:
		print "<script>alert('New Password Update');window.location.href='changepass.py';</script>"
	else:
		print "<script>alert('Not Updated');window.location.href='changepass.py';</script>"
else:
	print "<script>alert('New Password or old Updated is not matched');window.location.href='changepass.py';</script>"
